...
if __name__ == "__main__":
    ...
    tokenizer = BertTokenizer.from_pretrained(model_list[model_id], do_lower_case=False)
    ...
    if load_checkpoint and os.path.exists(output_dir + 'bert_crf_ner.checkpoint.pt'):
        ...
        checkpoint = torch.load(output_dir + 'bert_crf_ner.checkpoint.pt', map_location=device)
    ...
    for epoch in trange(start_epoch, total_train_epoch, desc='Epoch'):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step+1) % gradient_accumulation_steps == 0:
                ...
                torch.nn.utils.clip_grad_norm(parameters=model.parameters(), max_norm=1.0, norm_type=2)
                # update the params
                optimizer.step()
        ...
        if valid_f1 > valid_f1_pre:
            torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'valid_acc': valid_acc,
                       ...
                       os.path.exists(output_dir + 'bert_crf_ner.checkpoint.pt'))
    ...
    plt.plot(train_average_loss, 'b-o', 'train_average_loss')
    ...
    checkpoint = torch.load(output_dir + 'bert_crf_ner.checkpoint.pt', map_location='cpu')
